...
def tranformer_forward(
    ...
    **params: dict,
    ...
    if joint_attention_kwargs is not None :
      ...
      additional_kwargs["layers_list"] = joint_attention_kwargs['layers_list']
    ...
    num_layer=len(self.transformer_blocks)
    for index_block, block in enumerate(self.transformer_blocks):
        if self.training and self.gradient_checkpointing:
            ...
            encoder_hidden_states, hidden_states, condition_latents = block_forward(
                ...
                image_rotary_emb=image_rotary_emb,
                layers=index_block,
                additional_kwargs=additional_kwargs if (joint_attention_kwargs is None or index_block % joint_attention_kwargs[
                    'double_inject_blocks_interval'] == 0) else {k: v for k, v in additional_kwargs.items() if
    ...
    for index_block, block in enumerate(self.single_transformer_blocks):
        if self.training and self.gradient_checkpointing:
            ...
            result  = single_block_forward(
                ...
                temb=temb,
                image_rotary_emb=image_rotary_emb,
                layers=index_block+num_layer,
                additional_kwargs=additional_kwargs if (joint_attention_kwargs is None or index_block % joint_attention_kwargs[
                    'single_inject_blocks_interval'] == 0) else {k: v for k, v in additional_kwargs.items() if
                                                                k != 'regional_attention_mask'},
                # delete attention mask to avoid region control
                **(
                    {
